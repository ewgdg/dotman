...
- The selected package is still resolved into one final package before `apply`, `upgrade`, or `import`.
...
- Targets may define `render` as a forward transform used during `apply` and `upgrade`.
- `render` should be a non-interactive stdout producer.
- Targets may define `capture` as a non-interactive live-to-repo projection used during import planning.
- `capture` should be a non-interactive stdout producer.
- Targets may define `reconcile` as the actual import action used during `import`.
- `reconcile` may be interactive and should receive both repo and live paths.
- Targets may define `import_view_repo` to control how repo-side content is projected during import planning.
- Targets may define `import_view_live` to control how live-side content is projected during import planning.
- `import_view_repo` and `import_view_live` may use built-in values such as `raw`, `render`, and `capture`, or an explicit script/command string when needed.
...
- A template-style forward-managed target should typically set:
  - `import_view_repo = "render"`
  - `import_view_live = "raw"`
- A live-dump-style target should typically keep:
  - `import_view_repo = "raw"`
  - `import_view_live = "capture"`
- Targets may define `apply_ignore` as gitignore-style patterns relative to the source root.
...
- Standard `.gitignore` files inside the package source tree should still be respected during apply and upgrade.
- `apply` and `upgrade` should install everything under the source tree except paths excluded by `.gitignore` or `apply_ignore`.
...
- `render` is the forward path used during `apply` and `upgrade`.
- `capture` is the live-side planning projection used during `import`.
- `reconcile` is the reverse action used during `import`.
- Directory targets should not support `render`, `capture`, or `reconcile` in v1.
- During import planning, dotman should compare:
...
- Live file mode checks should compare against target `chmod` after both `apply` and `import`.
