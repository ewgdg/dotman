...
def test_same_live_path_is_allowed_when_repo_source_resolves_to_same_file(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    ...
    (repo_root / "profiles" / "default.toml").write_text("", encoding="utf-8")
    (repo_root / "groups" / "all.toml").write_text('members = ["alpha", "beta"]\n', encoding="utf-8")
    (repo_root / "shared.conf").write_text("shared = 1\n", encoding="utf-8")
    (repo_root / "packages" / "alpha" / "package.toml").write_text(
        "\n".join(
            [
                'id = "alpha"',
    ...
    plan = engine.plan_push_binding("fixture:all@default")

    assert len(plan.target_plans) == 2
    assert {target.package_id for target in plan.target_plans} == {"alpha", "beta"}
    assert {target.repo_path for target in plan.target_plans} == {repo_root / "shared.conf"}


def test_nested_live_paths_are_allowed_when_child_resolves_inside_same_repo_dir(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    ...
    (repo_root / "profiles" / "default.toml").write_text("", encoding="utf-8")
    (repo_root / "groups" / "all.toml").write_text('members = ["alpha", "beta"]\n', encoding="utf-8")
    (repo_root / "shared-dir").mkdir()
    (repo_root / "shared-dir" / "settings.toml").write_text("value = 1\n", encoding="utf-8")
    (repo_root / "packages" / "alpha" / "package.toml").write_text(
        "\n".join(
            [
                'id = "alpha"',
    ...
    (repo_root / "packages" / "beta" / "package.toml").write_text(
        "\n".join(
            [
                ...
                "",
                "[targets.settings]",
                'source = "../../shared-dir/settings.toml"',
    ...
    assert len(plan.target_plans) == 2
    assert {target.target_name for target in plan.target_plans} == {"app", "settings"}


def test_nested_live_paths_conflict_when_child_resolves_to_different_repo_file(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    home = tmp_path / "home"
    ...
    (repo_root / "shared-dir").mkdir()
    (repo_root / "shared-dir" / "settings.toml").write_text("value = 1\n", encoding="utf-8")
    (repo_root / "other-settings.toml").write_text("value = 2\n", encoding="utf-8")
    (repo_root / "packages" / "alpha" / "package.toml").write_text(
        "\n".join(
            [
                'id = "alpha"',
    ...
    with pytest.raises(
        ValueError,
        match=r"incompatible nested targets: alpha:app contains beta:settings",
    ):
        engine.plan_push_binding("fixture:all@default")
