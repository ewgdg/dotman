...
def test_push_cli_interactively_selects_ambiguous_tracked_binding(
    ...
    monkeypatch.setattr(
        ...
        lambda *, plans, operation, json_output, full_paths=False, unattended=False: True,
    )
...
def test_push_cli_combined_selection_menu_excludes_selected_targets_across_tracked_bindings(
    ...
    monkeypatch.setattr("sys.stdin.isatty", lambda: True)
    answers = iter(["s", "1"])
    monkeypatch.setattr(cli, "prompt", lambda _message, **_options: next(answers))
...
def test_push_cli_reviews_diffs_before_selection(
    ...
    def fake_run_diff_review_menu(
        ...
        *,
        ...
        recorded["unattended"] = unattended
        return True
...
def test_push_cli_skips_diff_review_for_json_output(
    ...
    monkeypatch.setattr(
        ...
        lambda review_items, *, operation, unattended=False: (_ for _ in ()).throw(AssertionError("review menu should not run")),
    )
